...
describe(`AsyncReasoner`, () => {
    ...
    it(`should answer with questions if max depth is 3`, async () => {
        ...
        expect(completions.map(({ completion }) => completion)).toEqual(expect.arrayContaining(expected));
    });
    it(`should trace query history for each completion`, async () => {
        const maxExploreDepth = 3;
        const config: ReasoningConfig = {
        ...
        expect(completions.map(({ queries }) => queries)).toEqual(expect.arrayContaining(expected));
    });
});
